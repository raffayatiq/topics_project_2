...
class State:
	def __init__(self, config, trace_file):
		self.trace_file = trace_file
                self.active_abr = self._ActiveABR()
	...
	def _InsertJoinTimeAndInitBW(self):
		t = []
		t.append(self.jointime)
		b = []
		b.append(self.BW)
		row = zip(t,b)
		return row + self.bw_array

        def _ActiveABR(self):
            abrs = [config.MPC_ABR, config.BOLA_ABR, config.HYB_ABR]
            if abrs.count(True) != 1:
                print >> sys.stderr, "None or more than one ABRs selected, check config.py"
                sys.exit()
            ret = ""
            if config.MPC_ABR:
                ret = "MPC"
        ...
        def GenerateStats(self):
            #avgbr = round(self.AVG_SESSION_BITRATE / float(self.PLAYTIME)/1000, 4) # add float
            avgbr = round(self.AVG_SESSION_BITRATE / float(config.TOTAL_CHUNKS * config.CHUNKSIZE)/1000, 4) # add float
            rebuf = self.BUFFTIME / float(self.BUFFTIME + self.PLAYTIME)
            avgrebuf = round(rebuf / float(config.TOTAL_CHUNKS), 4)
            avgchange = round(self.change_magnitude / float(config.TOTAL_CHUNKS), 4)
            qoe = round(avgbr / 1000.0 - avgrebuf * config.REBUF_PENALTY - avgchange * config.SMOOTH_PENALTY, 4)
            return qoe, avgbr, avgrebuf, avgchange
        

       # def generateStats(AVG_SESSION_BITRATE, BUFFTIME, PLAYTIME, bufftimems, playtimems, change_magnitude):
       #     AVG_SESSION_BITRATE = (AVG_SESSION_BITRATE/float(PLAYTIME)/1000) # add float
       #     REBUF_RATIO = round(BUFFTIME/float(BUFFTIME + PLAYTIME),3)
       #     rebuf_groundtruth = round(bufftimems/float(bufftimems + playtimems),3)
       #     print change_magnitude
